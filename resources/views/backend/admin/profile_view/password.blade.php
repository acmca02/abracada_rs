...
<div class="tab-pane fade show active" id="basicInfo" role="tabpanel" aria-labelledby="basicInfo-tab">
    ...
    <form action="{{ route('user.password.update') }}" method="post" enctype="multipart/form-data">
        ...
        <div class="fpb-7">
        <label for="pass" class="eForm-label">{{ get_phrase('mot de passe') }}</label>
        <input type="password"  class="form-control eForm-control" id="pass" name="password" placeholder="New Password" value="">
        </div>

        <div class="fpb-7">
        <label for="confirm_pass" class="eForm-label">{{ get_phrase('Confirmez le mot de passe') }}</label>
        <input type="password" class="form-control eForm-control" id="confirm_pass" name="password_confirmation" placeholder="Confirmez le mot de passe" value="">
        </div>
